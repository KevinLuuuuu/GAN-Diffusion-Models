...
class ImageDataset(Dataset):
    ...
    def __init__(
        self, data_path, transform, csv_path="", train_set = True):
        ...
        if self.train_set:
            file = open(self.csv_path)
            csvreader = csv.reader(file)
            header = next(csvreader)
            for image, label in csvreader:
                self.image_label.append((os.path.join(data_path, "data", image), label))
    ...
    def __getitem__(self,index):
        if self.train_set:
            image_name, label = self.image_label[index]
...
class Up(nn.Module):
    def __init__(self, in_channels, out_channels, emb_dim=256):
        ...
        self.up2 = nn.Upsample(scale_factor=2.5, mode="bilinear", align_corners=True)
